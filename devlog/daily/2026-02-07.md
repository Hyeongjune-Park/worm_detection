...
- [x] SAM2 잎맥 번짐 문제 대응 방안 설계 및 구현 → **MC1 Mask Cleanup 구현 완료**
- [ ] MC1 overlay 육안 검증 (MC1 단독, MC1+P2)
- [ ] K1, SM1 단일 토글 테스트 완료
- [ ] P2 + F3 조합 테스트
- [ ] MC1 + P2 + 추가 조합 테스트
...
### 4. MC1 Mask Cleanup Pipeline 구현

GPT+Claude 합의에 따라 "거부보다 정리(clean) 우선" 전략 구현.

#### 구현 내용 (`qa/mask_cleanup.py`)
새 토글 **MC1** (`mask_cleanup`) 추가. shape_score < 0.8 감지 시 3단계 정리:

1. **CCF (Connected Component Filtering)**: 마스크를 connected component로 분리, pred_center에 가장 가까운 것만 유지
2. **조건부 Erosion**: 컴포넌트가 1개(전부 연결)이면 erode→CCF→dilate로 얇은 연결 끊기
3. **Expected Region AND**: baseline 면적+종횡비로 타원 생성, 마스크와 AND (과도 확산 차단)

정리 후 ShapeStats 재분석 → shape_score 복원 시 fusion에서 SAM2 채택.

#### 파이프라인 흐름 변경
```
SAM2 mask → ShapeStats 1차 → shape_score < 0.8?
  YES → cleanup_mask() → rebuild_sensor_result() → ShapeStats 2차
  NO  → 그대로
→ fusion (SH1 gate 동일: < 0.6이면 reject)
```

#### 수정 파일
- **신규**: `qa/mask_cleanup.py` — cleanup_mask(), rebuild_sensor_result()
- **수정**: `config_toggles.py` — mask_cleanup 토글 추가
- **수정**: `config.yaml` — mask_cleanup: false 추가
- **수정**: `pipeline/runner.py` — c-3 단계 삽입 (ShapeStats 이후, fusion 전)

#### 테스트 결과

| 설정 | ACTIVE | OCCLUDED | UNCERTAIN | RESEED | 비고 |
|------|--------|----------|-----------|--------|------|
| Baseline | 383 | 176 | 14 | 1 | 기준 |
| MC1 단독 | **473 (+90)** | **51 (-125)** | 21 | 1 | OCCLUDED 대폭 감소 |
| P2 단독 (이전) | 526 (+143) | 96 | 3 | 1 | 이전 최고 |
| **MC1 + P2** | **569 (+186)** | 99 | 26 | 1 | **수치 최고** |

- MC1 단독: OCCLUDED 176→51로 71% 감소. 번짐 마스크 정리 후 shape_score 복원 성공
- MC1+P2: ACTIVE=569로 baseline 대비 +48.6% 개선
- **주의**: overlay 육안 검증 필요 (SH1_OFF 교훈)

#### 출력 파일 위치
- MC1 OFF: `output/2026-02-07/mc1_off/`
- MC1 ON: `output/2026-02-07/mc1_on/`
- MC1+P2: `output/2026-02-07/mc1_p2/`
